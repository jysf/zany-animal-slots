...
8. REQUIRED: capture a milestone accomplishment for the shipped stage —
   `just brag "<what this stage delivered>"` (or the `brag` CLI).
...
8. REQUIRED: capture a milestone accomplishment for the shipped project —
   `just brag "<what this project delivered>"` (or the `brag` CLI).
...
- REQUIRED: capture an accomplishment in the repo brag log —
  `just brag "<what this spec delivered>"` (or the `brag` CLI). This is
  the running app-level record (`./ACCOMPLISHMENTS.md`), separate from
  the per-spec Reflection. Don't skip it.
