...
- [~] SPEC-024 (verify) — **Win particle burst**: leaves 🍂 / acorns 🌰 erupt over the reels on a win, count scaled by tier (small<big<jackpot); CSS `particle-fly` keyframe, renders nothing under reduced motion, keyed off `celebration`. **[M]**
