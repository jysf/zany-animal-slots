...
- [~] SPEC-033 (verify) — **Colorblind-safe state cues**: win badge gains a tier WORD (WIN / BIG WIN / JACKPOT) + `data-tier` — a text cue backed by a redundant tier border color (win-tier tokens); symbols already shape-distinct (DEC-006). **[S]**
...
**Count:** 5 shipped / 0 active / 2 pending — 5×M, 1×(S–M), 1×S; **no L** (audio suite + reduced-motion + contrast/44px done; colorblind + perf remain). Sized at
