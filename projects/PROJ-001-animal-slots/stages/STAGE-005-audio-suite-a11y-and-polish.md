...
- [~] SPEC-034 (build) — **Performance pass**: a compositor-only keyframe guard (every `@keyframes` animates transform/opacity only) + a `will-change` hint on the spin + a documented measurement — validates DEC-004 holds at ~60fps. **[M]**
