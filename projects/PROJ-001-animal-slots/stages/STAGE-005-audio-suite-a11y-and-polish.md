...
- [~] SPEC-031 (build) — **Reduced-motion audit**: a global motion safety net + a regression-guard sweep (every `@keyframes` CSS has a reduced-motion block) + audio-not-motion-gated + App-renders-under-reduced-motion; confirms the paths shipped per-stage. **[S]**
