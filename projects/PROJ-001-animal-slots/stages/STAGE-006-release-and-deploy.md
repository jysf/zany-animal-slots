...
- [~] SPEC-035 (build) — **[REPO]** Security headers + cache policy (`public/_headers`): a tight CSP (`default-src 'self'`; `style-src` allows inline style *attributes* for the dynamic `--reel-index`/particle custom props; `img-src 'self' data:` if needed) + `X-Content-Type-Options: nosniff`, `frame-ancestors 'none'`, `Referrer-Policy`, `Permissions-Policy`, and immutable-vs-no-cache rules; a contract test on the file. **[M]**
- [ ] (not yet written) — **[REPO]** CI supply-chain gate: `npm audit --omit=dev` + a `license-policy` permissive-only check (MIT/Apache-2.0/BSD/ISC/Zlib), as a `just` recipe + a GitHub Actions job; passes on the current dep set (incl. `tone`). **[M]**
...
**Count:** 0 shipped / 0 active / 6 pending — 3×[REPO] (2×M, 1×S) buildable now,
3×[OPS] (1×M, 2×S) handoff. No L; within the 3–8 range. The agent will build the
three [REPO] specs this batch and stop at the handoff boundary.
