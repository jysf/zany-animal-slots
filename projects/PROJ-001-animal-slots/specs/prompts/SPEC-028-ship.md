# SPEC-028 — SHIP prompt (fresh session!)
...
```
Cycle: ship. PR #28 for SPEC-028 is approved (verified 2026-06-27).

Read files in order:
1. /AGENTS.md — esp. §4 (cost tracking), §8 (cycle model), §15 (ship rules)
2. /projects/PROJ-001-animal-slots/specs/SPEC-028-ambient-bed-and-audio-graph.md — the spec (all sections)
3. /projects/PROJ-001-animal-slots/specs/SPEC-028-ambient-bed-and-audio-graph-timeline.md — current timeline state
...
Before merging, mark ship `[~]` in:
  projects/PROJ-001-animal-slots/specs/SPEC-028-ambient-bed-and-audio-graph-timeline.md

Pre-ship checklist:
[ ] CI passing? (check all checks green on PR #28)
...
- Run: just advance-cycle SPEC-028 ship
- Run: just archive-spec SPEC-028  (moves spec + timeline into done/)
- REQUIRED: capture accomplishment — just brag "SPEC-028: ambient music bed + audio graph foundation — shared singleton audio graph (master Gain bus + bed/sfx/jingle channel gains) via audioEngine.ts; generative PolySynth Transport loop ambient bed via ambientBed.ts; useAmbientBed hook gated by muted+unlocked (injected-spy tests, no real Tone); jingle re-routed from toDestination() onto getChannel('jingle'); DEC-013 audio-graph architecture implemented; wired into App; engine untouched; 214/214 tests green (+11 tests); 1st of 7 SPECs in STAGE-005"

NOTE: This is NOT the last spec in STAGE-005 (1/7 — next is SPEC-029 SFX set).
...
Decisions referenced: DEC-013 (audio-graph architecture — master bus + named channels
+ Transport; this spec IS that implementation), DEC-007 (synthesized Tone.js audio,
gated by first-gesture unlock + persisted mute; ambient bed is synthesized,
no asset files), DEC-001 (engine unchanged; pure UI), DEC-007 and DEC-001 both
honored; no new DEC-* emitted by this build (DEC-013 authored at design).
```
